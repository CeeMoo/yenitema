			...
			<div id="forumposts">
				<div class="cat_bar">
					<h3 class="catbg">
						...
						<span id="top_subject" class="entry-title">', $txt['topic'], ': ', $context['subject'], '&nbsp;<span>(', $context['num_views_text'], ')</span></span>
  ...
  (
    function(maxht, maxwt, minht, minwt) {
      ...
      var resize_image = function(img, newht, newwt) {
        img.height = newht;
        img.width  = newwt;
        $(img).wrap(\'<div class="resized_image"><a href="\' + img.src + \'" target="_blank"></a></div>\');
        $(img).parent().before();
        $(img).parent().after();
      };
...
function template_single_post($message, $force_alternate = null)
{
	...
	echo '
						<div class="poster">
									<h4 class="p-author h-card" style="text-align:center; margin-bottom:5px;">';
	...
	// Show the user's avatar.
            ...
            if (!empty($settings['show_user_images']) && empty($options['show_no_avatars']) && !empty($message['member']['avatar']['image']))
                echo '
                                <li class="avatar">
                                    <a href="', $scripturl, '?action=profile;u=', $message['member']['id'], '">
                                         <img class="imgavatar" src="' . $message['member']['avatar']['href'] . '" width="100" height="100" alt="" />
                                    </a>
                                </li>';
                ...
                echo '
                                <li class="avatar">
                  <a href="', $scripturl, '?action=profile;u=', $message['member']['id'], '">
                    <img class="imgavatar" src="'.$settings['images_url'].'/default.png" alt="" />
                  </a>
                </li>';
            }
	...
	if (!$message['member']['is_guest'])
		echo '
								<li class="icons" style="text-align:center;">', $message['member']['group_icons'], '</li>';

	// Show the member's primary group (like 'Administrator') if they have one.
	if (!empty($message['member']['group']))
		echo '
								<li class="membergroup" style="text-align:center;">', $message['member']['group'], '</li>';
// Any custom fields to show as icons?
	...
	if (empty($message['member']['options']['teknoromi'])) {
                  $message['member']['options']['teknoromi']='Mesgul';
              }
              echo '
                 
  <li style="text-align:center;"><img src="', $settings['default_images_url'], '/teknoromi/', $message['member']['options']['teknoromi'], '.gif" /></li> ';
...
';
	echo '
	              ...
	              echo '	<ul><div class="displaymenu">
			<div class="displaymenu1"><li class="displaymenu2"><strong>Bilgiler</strong></li>';
	// Is karma display enabled?  Total or +/-?
		if ($modSettings['karmaMode'] == '1')
			echo '
								<li class="liclass">', $modSettings['karmaLabel'], ' ', $message['member']['karma']['good'] - $message['member']['karma']['bad'], '</li>';
	...
	if (!empty($message['member']['title']))
		echo '
								<li class="liclass">', $message['member']['title'], '</li>';
	...
	if (!$message['member']['is_guest'])
	{
		...
		if ((empty($settings['hide_post_group']) || $message['member']['group'] == '') && $message['member']['post_group'] != '')
			echo '
								<li class="liclass">', $message['member']['post_group'], '</li>';

		// Show how many posts they have made.
		if (!isset($context['disabled_fields']['posts']))
			echo '
								<li class="liclass"><strong>', $txt['member_postcount'], ': </strong>', $message['member']['posts'], '</li>';
		...
		if (!empty($settings['show_gender']) && $message['member']['gender']['image'] != '' && !isset($context['disabled_fields']['gender']))
			echo '
								<li class="liclass"><strong>', $txt['gender'], ': </strong>', $message['member']['gender']['image'], '</li>';

		// Show their personal text?
		if (!empty($settings['show_blurb']) && $message['member']['blurb'] != '')
			echo '
								<li class="liclass">', $message['member']['blurb'], '</li>';
	...
	echo '
								...
								<h5>
									<a href="', $message['href'], '" rel="nofollow" title="', !empty($message['counter']) ? sprintf($txt['reply_number'], $message['counter']) : '', ' - ', $message['subject'], '" style="font-size: 0.9em;" class="dt-published">', $message['time'], '</a>';
	...
	echo '
								<div class="entry-summary" id="msg_', $message['id'], '"', $ignoring ? ' style="display:none;"' : '', '>', $message['body'], '</div>
							</div>';
